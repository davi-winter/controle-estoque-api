...
namespace InventoryControl.Application.UseCases.StockMovements
{
    public class GetHistoryByProductIdUseCase
    {
        ...
        public async Task<Result<IEnumerable<StockMovementResponse>>> ExecuteAsync(Guid productId)
        {
            var stockMovements = await _repository.GetHistoryByProductIdAsync(productId);
